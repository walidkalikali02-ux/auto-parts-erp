  ...
  return (
    <div className="flex-1 p-8" style={{ maxWidth: 1100, margin: "0 auto", width: "100%" }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-arabic text-2xl font-bold" style={{ color: "var(--color-ink)" }}>العملاء</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-ink-muted)" }}>{loading ? "..." : `${customers.length} عميل`}</p>
        </div>
      ...
      <div className="card p-4 mb-6 flex gap-3">
        <div className="flex-1 relative">
          ...
          <input className="input" style={{ paddingRight: 36 }} placeholder="بحث بالاسم أو الجوال..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <button className="btn btn-primary" onClick={openAdd}><span className="font-arabic">إضافة أول عميل</span></button>
          </div>
        ) : (
          <table className="erp-table">
            <thead><tr><th>الاسم</th><th>النوع</th><th>الجوال</th><th>المدينة</th><th>الرصيد</th><th>حد الائتمان</th><th></th></tr></thead>
            <tbody>
              {customers.map((c) => {
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editCustomer ? "تعديل العميل" : "إضافة عميل جديد"} size="md">
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
            {field("الاسم بالعربي *", "name_ar", { required: true, placeholder: "محمد العتيبي" })}
            {field("الاسم بالإنجليزي", "name", { placeholder: "Mohammed Al-Otaibi" })}
            {field("رقم الجوال", "phone", { placeholder: "0501234567", type: "tel" })}
            {field("البريد الإلكتروني", "email", { placeholder: "customer@email.com", type: "email" })}
            {field("المدينة", "city", { placeholder: "الرياض" })}
            {field("الرقم الضريبي", "tax_number", { placeholder: "300000000000003" })}
            <div>
              <label className="block font-arabic text-sm font-medium mb-1.5" style={{ color: "var(--color-ink-2)" }}>نوع العميل</label>
              <select className="input" value={form.customer_type} onChange={(e) => set("customer_type", e.target.value)}>
                <option value="retail">تجزئة</option><option value="wholesale">جملة</option><option value="workshop">ورشة</option>
              </select>
            </div>
            {field("حد الائتمان (ر.س)", "credit_limit", { type: "number", min: "0", placeholder: "0" })}
          </div>
          ...
          <div className="flex gap-3 justify-end pt-2" style={{ borderTop: "1px solid var(--color-border-light)" }}>
            <button type="button" className="btn btn-outline" onClick={() => setShowForm(false)}><span className="font-arabic">إلغاء</span></button>
            <button type="submit" className="btn btn-primary" disabled={saving}>
              ...
              <span className="font-arabic">{editCustomer ? "حفظ" : "إضافة"}</span>
            </button>
          </div>
        </form>
      </Modal>
